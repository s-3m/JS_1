...
function handleClick(clickedButtonEvent) {
    ...
    let butTxt = clickedButtonEvent.target.innerText;
    
    if (butTxt === 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (butTxt === 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let txt = 'Что-то с чем-то и еще чего-нибудь';// 6.1 сохраняем произвольный текст в переменную
    
    card.wrap.querySelector('.productName').insertAdjacentHTML("afterend", `<div class = "desc">${txt}</div>`);// 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
